...
require 'optim'
require 'HypernymImage'
local config = { learningRate = args.lr }

local lookupTableWeights = torch.load('weights.t7')
local hypernymNet = nn.HypernymImage(4096, 50, lookupTableWeights, datasets.numEntities)
    ...
    if count % args.eval_freq == 0 then
        ...
        for name, dataset in pairs(datasets_eval) do
            ...
            if not best_accuracies[name] or real_accuracy > best_accuracies[name] then
                ...
                saved_weight = hypernymNet.visualFeatureModule.weight:float()
            end
        end
    end
end
...
torch.save('visual_embedding_weights_vs.t7', saved_weight)
